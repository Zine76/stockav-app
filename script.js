// --- START OF FILE script.js (CORRECTED WITH Supabase Kit Storage) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- SUPPRIMÉ --- La constante pour localStorage n'est plus utilisée ici
    // const KIT_LOCALSTORAGE_KEY = 'stockav_currentKit';
    ...
    // --- AJOUTÉ : Nouvelle fonction pour sauvegarder le kit dans Supabase ---
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .upsert({
                    ...
                    // updated_at est géré par le trigger DB (si créé) ou automatiquement par upsert
                }, {
            ...
            if (error) {
                ...
                // Peut-être afficher une erreur à l'utilisateur dans le feedback kit ?
                showKitFeedback("Erreur sauvegarde du kit.", 'error');
            } else {
                console.log("Kit sauvegardé avec succès dans Supabase.");
    ...
    // --- AJOUTÉ : Nouvelle fonction pour charger le kit depuis Supabase ---
    async function loadKitFromSupabase() {
        currentKitSelection = []; // Réinitialiser d'abord
        if (!currentUser || !supabase) {
            console.log("Chargement du kit annulé : Utilisateur non connecté ou Supabase non prêt.");
            return;
        }
        console.log(`Chargement du kit depuis Supabase pour user ${currentUser.id}...`);
        try {
            ...
            if (error) {
                // Gérer les erreurs potentielles
                 if (error.message.includes('relation "public.user_kits" does not exist')) {
                     ...
                     // Bloquer potentiellement l'utilisation du kit ?
                 } else if (error.message.includes('violates row-level security policy')) {
                     console.error("Erreur RLS : L'utilisateur n'a pas la permission de lire ce kit (ne devrait pas arriver).", error);
                     showKitFeedback("Erreur de permission (lecture kit).", 'error');
                 } else {
                     console.error("Erreur lors du chargement du kit depuis Supabase:", error);
                     showKitFeedback("Erreur chargement du kit.", 'error');
                 }
                 return; // Sortir si erreur
            }

            if (data && data.kit_data && Array.isArray(data.kit_data)) {
                currentKitSelection = data.kit_data;
                console.log(`Kit chargé depuis Supabase (${currentKitSelection.length} éléments).`);
            } else if (data && !data.kit_data) {
                 console.log("Enregistrement kit trouvé mais kit_data est null ou vide.");
                 currentKitSelection = [];
            } else {
            ...
            console.error("Erreur JS inattendue lors du chargement du kit:", err);
    ...
    // --- AJOUTÉ : Nouvelle fonction pour supprimer/vider le kit dans Supabase ---
    ...
    // --- AJOUTÉ : Helper pour afficher feedback dans la vue Kit (index.html) ---
    function showKitFeedback(message, type = 'info', duration = 3000) {
     ...
     // --- AJOUTÉ : Helper pour rafraîchir l'UI liée au kit (tableau inventaire et vue kit) ---
     async function refreshKitRelatedUI() {
         ...
         if (inventoryView.classList.contains('active-view')) {
             await displayInventory(currentInventoryPage); // Re-display pour mettre à jour coches et styles
         }
    ...
    async function handleLogout() {
        ...
        if (error) {
            ...
            console.log("Déconnexion Supabase réussie.");
             ...
             // --- MODIFIÉ --- : Vider la variable locale du kit aussi lors de la déconnexion
             currentKitSelection = [];
    ...
    // --- MODIFIÉ --- : setupAuthListener, handleUserConnected, handleUserDisconnected pour charger/vider le kit
    async function setupAuthListener() {
        ...
        try {
            ...
            activeSession = session; // Mettre à jour la session active dès le départ
            ...
            isInitialAuthCheckComplete = true; // Marquer comme complet même en cas d'erreur
            handleUserDisconnected(true); // Fallback en état déconnecté
        }
        ...
        supabase.auth.onAuthStateChange(async (event, session) => {
             ...
             activeSession = session; // Toujours mettre à jour la session active
            ...
            switch (event) {
                case 'SIGNED_IN':
                    ...
                    if (session) await handleUserConnected(session.user, false); // isInitialLoad = false
                ...
                case 'SIGNED_OUT':
                    ...
                    handleUserDisconnected(false); // isInitialLoad = false
                    break;
                case 'INITIAL_SESSION':
                     console.log("   [onAuthStateChange] Initial session event received.");
                     // Normalement, l'état initial est déjà géré par getSession,
                     // mais on peut ajouter une vérification de cohérence ici si nécessaire.
                     // Si la session existe mais que currentUser est null (ou différent), on connecte.
                     if (session && (!currentUser || currentUser.id !== session.user.id)) {
                         ...
                         await handleUserConnected(session.user, true); // Considéré comme chargement initial pour recharger données
                     }
                     // Si la session est null mais que currentUser existe, on déconnecte.
                     else if (!session && currentUser) {
                         console.log("   [onAuthStateChange] Correcting state based on INITIAL_SESSION: Disconnecting user.");
                         handleUserDisconnected(true); // Considéré comme chargement initial
                     } else {
                 ...
                 case 'TOKEN_REFRESHED':
                    ...
                    // On vérifie si la session existe toujours et si l'utilisateur correspond
                ...
                case 'USER_UPDATED':
                    ...
                    if (session) {
                         // Retraiter comme une connexion pour mettre à jour les infos utilisateur et potentiellement recharger les données si l'email/code a changé
                         await handleUserConnected(session.user, false);
                    }
                    break;
    ...
    async function handleUserConnected(user, isInitialLoad) {
         ...
         const previousUserCode = currentUserCode; // Sauvegarder l'ancien code pour comparaison
         ...
         // --- MODIFIÉ --- : Logique de chargement du kit et rechargement des données
         if (userChanged || isInitialLoad) {
             console.log("Chargement données/caches suite à connexion/changement user...");

             // 1. Charger le kit depuis Supabase pour le nouvel utilisateur
             console.log("Tentative de chargement du kit depuis Supabase...");
             await loadKitFromSupabase(); // Charge currentKitSelection depuis la DB
             ...
             // 3. Afficher message de bienvenue si l'utilisateur a changé et on est sur la vue recherche
             ...
             if (currentActiveView) {
                 await reloadActiveViewData(currentActiveView); // Met à jour l'UI, y compris les coches du kit dans l'inventaire
             } else {
                 // Si aucune vue active (peut arriver au premier chargement)
            ...
            // Même si l'utilisateur n'a pas changé, on rafraîchit l'UI liée au kit au cas où
            // elle aurait été modifiée dans un autre onglet pendant que celui-ci était inactif.
             await refreshKitRelatedUI();
         }
    }
    function handleUserDisconnected(isInitialLoad) {
         ...
         // --- MODIFIÉ --- : Vider la variable kit locale lors de la déconnexion
         currentKitSelection = [];
         ...
         if (isProtectedViewActive) {
             ...
             setActiveView(searchView, searchTabButton); // Pas besoin d'await ici
             ...
             setActiveView(searchView, searchTabButton);
              ...
              // Si on était sur l'inventaire, recharger pour masquer coches/tiroirs
    ...
    // ... (reloadActiveViewData - légèrement adapté pour le kit, showGenericError, clearProtectedViewData, setActiveView inchangés) ...
    async function reloadActiveViewData(viewElement) {
        if (!viewElement) return;
         ...
         try {
             switch(viewId) {
                case 'inventory-view':
                    // Assurer que les catégories sont chargées si user connecté
                    if (categoriesCache.length === 0 && currentUser) await getCategories();
                    await populateInventoryFilters();
                    // displayInventory mettra à jour les coches en fonction de currentKitSelection
                ...
                case 'bom-view': // Géré par refreshKitRelatedUI ou handleUserConnected/Disconnected
                    if (currentUser) displayCurrentKitDrawers(); // Appel direct pour s'assurer de l'affichage
    ...
    async function setActiveView(viewToShow, buttonToActivate){
        ...
        // Éviter de changer si déjà active
        if (viewToShow.classList.contains('active-view')) {
             console.log(`Vue ${viewToShow.id} déjà active.`);
             // Forcer un rechargement des données si nécessaire ? Non, géré par reloadActiveViewData appelé plus tard
             return;
        }
        ...
        // Charger/Recharger les données pour la nouvelle vue active
    ...
    // --- displayInventory : Utilise la variable globale currentKitSelection pour cocher les cases
    ...
    // --- LOGIQUE HISTORIQUE ---
    // ... (displayLog, formatLogTimestamp, addLogEntry inchangés - addLogEntry reste déconseillé) ...
    ...
    // --- MODIFIÉ --- : addStockEventListeners pour potentiellement maj kit si item modifié/supprimé
    // --- MODIFIÉ --- : handleExportCriticalStockTXT (juste pour la forme, pas de modif réelle)
    ...
    function addStockEventListeners() {
         ...
         updateQuantityButton?.addEventListener('click', async () => {
              // --- MODIFIÉ --- : Mettre à jour le kit si nécessaire après RPC
              ...
              try { console.log(`Calling RPC update_stock_and_log for ${ref}, change: ${change}`); const { data: newQuantity, error: rpcError } = await supabase.rpc('update_stock_and_log', { p_ref: ref, p_quantity_change: change, p_user_id: currentUser.id, p_user_code: currentUserCode, p_action_type: 'Admin Adjust Qty' }); if (rpcError) { if (rpcError.message.includes('new_quantity_below_zero')) throw new Error("Stock insuffisant pour ce retrait."); if (rpcError.message.includes('component_not_found')) throw new Error("Composant non trouvé (peut-être supprimé ?)."); throw new Error(`Erreur RPC: ${rpcError.message} (Code: ${rpcError.code})`); } showAdminFeedback(`Quantité pour "${ref}" mise à jour. Nouvelle quantité: ${newQuantity}.`, 'success'); currentQuantitySpan.textContent = newQuantity; quantityChangeInput.value = 0; if (componentInitialQuantityInput) componentInitialQuantityInput.value = newQuantity;
                  // Mettre à jour les vues et 7seg (inchangé)
                  ...
                  // --- AJOUT --- : Mettre à jour l'item dans le kit local et sauvegarder en DB
                  const kitIndex = currentKitSelection.findIndex(k => k.ref === ref);
                  if (kitIndex > -1) {
                      currentKitSelection[kitIndex].quantity = newQuantity;
                      await saveKitToSupabase(); // Sauvegarder le kit mis à jour
                      if (kitView.classList.contains('active-view')) displayCurrentKitDrawers(); // Rafraîchir UI Kit
                  }
         ...
         stockForm?.addEventListener('submit', async (event) => {
              // --- MODIFIÉ --- : Mettre à jour le kit si l'item modifié y était
             ...
             // --- AJOUTÉ --- : La quantité n'est PAS définie lors d'un upsert d'édition ! Elle est gérée par RPC.
             // Elle est incluse SEULEMENT lors de la création initiale.
             ...
             try {
                  // Upsert pour créer ou mettre à jour les métadonnées (hors quantité si édition)
                 ...
                 if (upsertError) { /* ... gestion erreur upsert inchangée ... */ if (upsertError.message.includes('violates foreign key constraint') && upsertError.message.includes('category_id')) { const failedCategoryOption = [...componentCategorySelectAdmin.options].find(opt => opt.value === categoryId); const failedCategoryName = failedCategoryOption ? failedCategoryOption.textContent : categoryId; throw new Error(`La catégorie sélectionnée "${failedCategoryName}" n'existe pas ou plus.`); } throw new Error(`Erreur base de données lors de l'enregistrement: ${upsertError.message} (Code: ${upsertError.code})`); }

                 // --- AJOUTÉ --- Si c'était une édition, il faut peut-être mettre à jour la quantité via RPC
                 let finalQuantity = quantity; // Quantité par défaut (si ajout)
                 if (isEditing) {
                     const currentDbQuantity = parseInt(currentQuantitySpan.textContent, 10);
                     const quantityInForm = parseInt(quantityRaw, 10);
                     // Si la quantité dans le formulaire a changé par rapport à ce qui était affiché
                     if (!isNaN(currentDbQuantity) && !isNaN(quantityInForm) && quantityInForm !== currentDbQuantity) {
                         ...
                         if (rpcError) { /* ... gestion erreur RPC comme dans updateQuantityButton ... */ if (rpcError.message.includes('new_quantity_below_zero')) throw new Error("Stock insuffisant (ajustement formulaire)."); if (rpcError.message.includes('component_not_found')) throw new Error("Composant non trouvé (ajustement formulaire)."); throw new Error(`Erreur RPC (ajustement formulaire): ${rpcError.message}`); }
                         finalQuantity = updatedQtyRpc; // Utiliser la quantité retournée par RPC
                         console.log(`Quantité ajustée via RPC (formulaire): ${finalQuantity}`);
                     } else {
                         finalQuantity = currentDbQuantity; // Pas de changement de quantité détecté dans le formulaire
                     }
                 } else {
                    // Si c'est un nouvel ajout avec quantité initiale > 0, logguer l'ajout initial
                    if (finalQuantity > 0) {
                        console.log(`Log manuel de l'ajout initial de ${finalQuantity} pour ${ref} via RPC...`);
                        const { error: logError } = await supabase.rpc('update_stock_and_log', {
                            p_ref: ref,
                            p_quantity_change: 0, // Le stock est déjà à `finalQuantity`, on ajoute juste le log
                            p_user_id: currentUser.id,
                            p_user_code: currentUserCode,
                            p_action_type: 'Admin Initial Add',
                            p_force_log: true, // Assurer que le log est créé même si change=0
                            p_initial_quantity: finalQuantity // Passer la quantité pour le log
                        });
                 ...
                 // Mettre à jour vues et 7seg (inchangé)
                 if (inventoryView.classList.contains('active-view')) await displayInventory(1);
                 if (auditView.classList.contains('active-view')) await displayAudit();
                 if (logView.classList.contains('active-view')) await displayLog(1);
                 await updateSevenSegmentForComponent(ref);

                 // --- AJOUT --- : Mettre à jour/supprimer l'item dans le kit local et sauvegarder en DB
                 const kitIndex = currentKitSelection.findIndex(k => k.ref === ref);
                 if (kitIndex > -1) {
                     // Récupérer les données complètes à jour de l'item (incluant la nouvelle quantité)
                     ...
                     if (updatedItemForKit) {
                         ...
                         // L'item n'existe plus ? (Ne devrait pas arriver ici mais par sécurité)
         ...
         deleteComponentButton?.addEventListener('click', async () => {
             ...
             // --- AJOUT --- : Retirer du kit AVANT de supprimer de l'inventaire
             ...
             try {
                 // Suppression log et inventaire (inchangée)
                 ...
                 // --- AJOUT --- : Sauvegarder le kit seulement s'il a été modifié
                 if (kitWasModified) {
                     await saveKitToSupabase();
                 }

                 // Réinitialisation et mise à jour UI (inchangée)
                 resetStockForm();
    ...
    // --- MODIFIÉ --- : handleQuantityResponse pour maj kit local et DB
    // ... (resetConversationState, getStockInfoFromSupabase inchangés) ...
    async function addMessageToChat(sender, messageContent, isHTML = false) {
        ...
        if (isHTML) {
            // Basic sanitization (could be improved with a library if needed)
        ...
        responseOutputChat.scrollTop = 0; // Scroll to top to see latest message
    }
    function displayWelcomeMessage() {
     ...
     async function handleUserInput() {
        ...
        if (!userMessageContent) return; // Ignorer si vide

        addMessageToChat('user', userMessageContent); // Ajouter le message utilisateur au chat
        componentInputChat.value = ''; // Vider l'input immédiatement après récupération
        ...
        responseOutputChat.scrollTop = 0; // Remonter en haut du chat

        try {
            // --- LOGIQUE AMÉLIORÉE : Détecter l'intention ---

            // 1. Vérifier si on attend une confirmation de quantité (prioritaire)
            if (conversationState.awaitingQuantityConfirmation) {
                ...
                // Le finally s'occupera de réactiver les inputs
                return; // Sortir car la réponse a été gérée
            }

            // 2. Essayer d'extraire une référence
            const potentialRef = extractReference(userMessageContent);

            // 3. Déterminer l'intention : Recherche vs Question Générale
            let isComponentSearch = false;
            if (potentialRef) {
                // *** HEURISTIQUE PLUS STRICTE ***
                ...
                if (simpleSearchPattern.test(userMessageContent)) {
                    ...
                    // Tout autre message contenant une référence est traité comme une question générale
                    isComponentSearch = false;
                    console.log("Intention détectée : Question Générale (Référence trouvée mais message plus complexe).");
                }
            } else {
                // Pas de référence -> Question Générale
                isComponentSearch = false;
                console.log("Intention détectée : Question Générale (pas de référence).");
            }

            // 4. Agir selon l'intention détectée
            // Reset l'état spécifique à la quantité/choix, pas l'historique général
            resetConversationState();

            if (isComponentSearch && potentialRef) {
                // --- Action : Recherche Composant / Équivalents ---
                loadingIndicatorChat.querySelector('i').textContent = `Recherche ${potentialRef}...`;
                conversationState.originalRefChecked = potentialRef; // Mémoriser la ref cherchée
                await checkComponentWithAI(potentialRef); // Appel fonction spécifique équivalents
            } else {
                // --- Action : Chat Général ---
                loadingIndicatorChat.querySelector('i').textContent = 'StockAV réfléchit...';
                const HISTORY_LENGTH = 10; // Nombre de messages d'historique à envoyer
                const recentHistory = chatHistory.slice(-HISTORY_LENGTH);
                console.log("Sending history to ai-general-chat:", recentHistory);

                // Appel à la fonction Edge 'ai-general-chat'
                const { data: chatData, error: chatError } = await supabase.functions.invoke('ai-general-chat', {
                    body: {
                        query: userMessageContent, // La nouvelle question
                        history: recentHistory      // Les messages précédents
                    }
                });
                ...
                // Gérer l'erreur retournée explicitement par la fonction Edge (dans la propriété error)
                if (chatData.error) {
                ...
                if (chatData.reply) {
                    await addMessageToChat('ai', chatData.reply, false); // isHTML = false
                } else {
    ...
    async function checkComponentWithAI(originalRef) {
        ...
        try {
            // 1. Vérifier le stock local pour l'original
            originalStockInfo = await getStockInfoFromSupabase(originalRef);
            await delay(150);

            // Mettre à jour 7-segment basé sur l'original trouvé localement
            if (originalStockInfo) {
                updateSevenSegmentForComponent(originalStockInfo.ref);
            } else {
                updateSevenSegmentForComponent(null);
            }
            ...
            if (originalStockInfo) {
                ...
                if (originalStockInfo.quantity > 0) {
                    ...
                    originalStatusHTML = `${indicatorHTML} Original <strong>${originalRef}</strong> : Rupture stock local.`;
                }
                ...
                originalStatusHTML = `${createStockIndicatorHTML(undefined, undefined)} Original <strong>${originalRef}</strong> : Non trouvé dans le stock local.`;
            }
            responseHTML += originalStatusHTML;

            // 2. Appeler l'IA pour les équivalents
            if (supabase) {
                ...
                try {
                    ...
                    const { data: aiResult, error: edgeError } = await supabase.functions.invoke('ai-component-info', {
                         body: { reference: originalRef }
                     });
                    ...
                    console.error("Erreur fonction Edge IA:", aiError);
                }
            } else {
                aiError = "Client Supabase non initialisé, recherche IA impossible.";
            }

            // 3. Vérifier le stock local des équivalents
            ...
            if (equivalents.length > 0 && supabase) {
                ...
                if (equivalentRefs.length > 0) {
                    const stockCheckPromises = equivalentRefs.map(async ref => {
                        const stockInfo = await getStockInfoFromSupabase(ref);
                        return { ref, stockInfo };
                    });
                    const results = await Promise.all(stockCheckPromises);
                    results.forEach(({ ref, stockInfo }) => {
                        if (stockInfo) { equivalentsStockInfo[ref] = stockInfo; }
                    });
                    console.log("Stock local des équivalents:", equivalentsStockInfo);
                }
            }

            // 4. Construire la réponse HTML pour les équivalents
                ...
                equivalents.forEach(eq => {
                     ...
                     if (eqStock) {
                         ...
                         if (eqStock.quantity > 0) {
                             ...
                             if (currentUser) {
                                 responseHTML += ` <button class="choice-button take-button" data-ref="${eq.ref}" data-qty="${eqStock.quantity}" data-threshold="${eqStock.critical_threshold ?? ''}" title="Sélectionner cet équivalent">Prendre</button>`;
                             }
                         } else {
                             responseHTML += ` : Rupture stock local.`;
                             responseHTML += provideExternalLinksHTML(eq.ref, true);
                         }
                     } else {
                         responseHTML += ` : Non trouvé localement.`;
                         responseHTML += provideExternalLinksHTML(eq.ref, true);
                     }
                     responseHTML += `</div>`;
                 });
                 ...
                 responseHTML += "<br/><br/>L'IA n'a pas suggéré d'équivalents.";
            }

            // 5. Ajouter le bouton "Prendre" pour l'original
            if (originalStockInfo && originalStockInfo.quantity > 0) {
                if (currentUser) {
                    ...
                    responseHTML += `<br/><br/><i>Original disponible. Connectez-vous pour pouvoir le prendre.</i>`;
                }
            }

            // 6. Ajouter liens externes pour l'original si besoin
            if (!originalStockInfo || originalStockInfo.quantity <= 0) {
            ...
            if (aiError) {
                responseHTML += `<br/><br/><i style="color: var(--error-color);">Erreur IA équivalents: ${aiError}.</i>`;
            }

            // 8. Message final ou état d'attente
            if (!conversationState.awaitingEquivalentChoice && !conversationState.awaitingQuantityConfirmation) {
                responseHTML += "<br/><br/>Autre recherche ?";
                resetConversationState();
            } else if (conversationState.awaitingEquivalentChoice && !currentUser) {
                 responseHTML += `<br/><br/><i>Connectez-vous pour pouvoir choisir et prendre un composant.</i>`;
            }
    ...
    async function handleQuantityResponse(userInput) {
        // --- MODIFIÉ --- : Mettre à jour le kit local et DB après la prise
        ...
        if (!conversationState.awaitingQuantityConfirmation || !ref || !currentUser) {
            ...
            if (!conversationState.awaitingQuantityConfirmation) { await handleUserInput(); } // Retraiter
            return;
        }
        ...
        if (isNaN(quantityToTake) || quantityToTake <= 0) {
            ...
            return; // Attendre réponse valide
        }
        ...
        try {
            ...
            const { data: newQuantity, error: rpcError } = await supabase.rpc('update_stock_and_log', {
                p_ref: ref, p_quantity_change: change, p_user_id: currentUser.id,
                p_user_code: currentUserCode, p_action_type: 'Chat Take Qty'
            });
            if (rpcError) { /* ... gestion erreur RPC inchangée ... */ if (rpcError.message.includes('new_quantity_below_zero')) throw new Error("Stock devenu insuffisant (vérification RPC)."); if (rpcError.message.includes('component_not_found')) throw new Error("Composant non trouvé lors de la mise à jour."); throw new Error(`Erreur RPC: ${rpcError.message}`); }
            ...
            // Mettre à jour les vues (inchangé)
            if (inventoryView.classList.contains('active-view')) await displayInventory(currentInventoryPage);
            if (auditView.classList.contains('active-view')) await displayAudit();
            if (logView.classList.contains('active-view')) await displayLog(1);
            if (lastDisplayedDrawerRef === ref) await updateSevenSegmentForComponent(ref); // MàJ 7seg
    ...
    async function getStockInfoFromSupabase(ref) {
         ...
         try {
             ...
             // Inclure category_id directement
             const { data, error } = await supabase
                 .from('inventory')
                 .select(`*, categories ( name )`) // Sélectionne tout + nom catégorie
                 .eq('ref', upperRef)
                 .maybeSingle();
             if (error && error.code !== 'PGRST116') { throw new Error(`Erreur base de données getStockInfo: ${error.message}`); }
             if (data) {
                 data.category_name = data.categories?.name || null; // Ajouter category_name si la jointure a réussi
                 if (data.category_id) data.category_id = String(data.category_id); // Assurer que category_id est une chaîne
                 delete data.categories; // Nettoyer l'objet retourné
    ...
    // --- MODIFIÉ --- : listener Confirm pour maj kit local et DB
    ...
    modalConfirmButton?.addEventListener('click', async () => {
        // --- MODIFIÉ --- : Mettre à jour le kit local et DB
        ...
        try {
            ...
            const { data: newQuantity, error: rpcError } = await supabase.rpc('update_stock_and_log', {
                p_ref: ref, p_quantity_change: change, p_user_id: currentUser.id,
                p_user_code: currentUserCode, p_action_type: 'Modal Adjust'
            });
            if (rpcError) { /* ... gestion erreur RPC inchangée ... */ if (rpcError.message.includes('new_quantity_below_zero')) throw new Error("Stock insuffisant (vérification RPC)."); if (rpcError.message.includes('component_not_found')) throw new Error("Composant non trouvé (peut-être supprimé ?)."); throw new Error(`Erreur RPC: ${rpcError.message}`); }
        ...
        } catch (err) { /* ... gestion erreur modale inchangée ... */ console.error("Erreur lors de la confirmation de la modale:", err); if (modalFeedback) { modalFeedback.textContent = `Erreur: ${err.message}`; modalFeedback.className = 'modal-feedback error'; } else { showGenericError(`Erreur modale (${ref}): ${err.message}`); } modalConfirmButton.disabled = (currentModalChange === 0); modalCancelButton.disabled = false; const potentialQtyBeforeError = modalInitialQuantity + currentModalChange; modalDecreaseButton.disabled = potentialQtyBeforeError <= 0; modalIncreaseButton.disabled = false; }
    });
    ...
    // --- MODIFIÉ --- : handleAdjustStock pour maj kit local et DB
    ...
    async function handleAdjustStock(row, button, input) {
        ...
        try {
            ...
            const { data: newQuantity, error: rpcError } = await supabase.rpc('update_stock_and_log', {
                p_ref: ref, p_quantity_change: difference, p_user_id: currentUser.id,
                p_user_code: currentUserCode, p_action_type: 'Audit Adjust'
            });
    ...
    // --- MODIFIÉ --- : handleKitCheckboxChange pour sauvegarder dans Supabase
    // --- MODIFIÉ --- : handleClearKit pour vider dans Supabase
    // displayCurrentKitDrawers, handleDrawerButtonClick restent inchangés pour la logique d'affichage locale
    function handleKitCheckboxChange(event) {
        ...
        if (!ref || !row || !row.dataset.itemData) { /* ... gestion erreur inchangée ... */ console.error("Données manquantes (ref ou itemData) sur la ligne pour la checkbox du kit.", checkbox, row); checkbox.checked = !checkbox.checked; showGenericError("Erreur lors de la sélection du composant pour le kit."); return; }
        ...
        try {
            ...
            if (checkbox.checked) {
                // Ajouter au kit local si pas déjà présent
            ...
            // --- MODIFIÉ --- : Sauvegarder dans Supabase si le kit a été modifié
            if (kitWasModified) {
                saveKitToSupabase(); // Appel asynchrone, mais on ne bloque pas l'UI ici
            }

            // Mettre à jour la vue Kit (index.html) si elle est active (inchangé)
            if (kitView?.classList.contains('active-view')) {
                displayCurrentKitDrawers();
            }
        } catch (error) { /* ... gestion erreur inchangée ... */ console.error(`Erreur lors de la gestion de la sélection kit pour ${ref}:`, error); checkbox.checked = !checkbox.checked; showGenericError(`Erreur lors de la sélection de ${ref} pour le kit: ${error.message}`); }
    }
    function displayCurrentKitDrawers() {
        // --- Affichage basé sur currentKitSelection (logique inchangée) ---
        ...
        // Logique de groupement et tri (inchangée)
        ...
        // Création des boutons (inchangée)
        sortedDrawers.forEach(drawerKey => {
            ...
            // On ne gère pas l'état 'collected' ici, c'est purement visuel et local à bom.html
            currentKitDrawersDiv.appendChild(button);
        });

        // Gestion feedback (inchangée)
    ...
    function handleDrawerButtonClick(event) {
        // --- Gère juste le style local ici, pas de persistance ---
    ...
    async function handleClearKit() { // Transformé en async pour clearKitInSupabase
        if (!currentUser) {
    ...
    function initializeApp() {
        ...
        // ... (tous les écouteurs existants restent inchangés, sauf ceux liés au kit qui sont modifiés) ...
        ...
        inventoryTableBody?.addEventListener('change', handleKitCheckboxChange); // Gère ajout/retrait et sauve dans Supabase
        ...
        currentKitDrawersDiv?.addEventListener('click', handleDrawerButtonClick); // Gère style local 'collected'
        clearKitButton?.addEventListener('click', handleClearKit); // Gère vidage variable locale et Supabase

        // Écouteur pour boutons "Prendre" dans le chat (inchangé)
        responseOutputChat?.addEventListener('click', async (event) => {
            // ... (logique inchangée, handleQuantityResponse mettra à jour le kit local+DB) ...
            ...
            if (targetButton && conversationState.awaitingEquivalentChoice) {
                ...
                // Mettre à jour l'état pour demander la quantité
        ...
        setupAuthListener(); // handleUserConnected chargera le kit depuis Supabase
...
// --- END OF FILE script.js (CORRECTED WITH Supabase Kit Storage) ---