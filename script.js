...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleLogin() {
        ...
        try {
            ...
            if (error) {
                ...
                // Charger les catégories dès la connexion réussie si le cache est vide
    ...
    async function handleLogout() {
        ...
        if (error) {
            ...
            console.log("Déconnexion Supabase réussie.");
             ...
             // Vider le cache des catégories à la déconnexion
    ...
    async function setupAuthListener() {
        ...
        try {
            ...
            if (session) {
                ...
                // Charger les catégories ici aussi si une session existe au chargement
                if (categoriesCache.length === 0) {
                    await getCategories();
                }
        ...
        supabase.auth.onAuthStateChange(async (event, session) => { // Ajout de async ici
            ...
            switch (event) {
                case 'SIGNED_IN':
                    // Charger les catégories si nécessaire après connexion
                    if (categoriesCache.length === 0) {
                       await getCategories();
                    }
                ...
                case 'TOKEN_REFRESHED':
                    ...
                    if (session && currentUser && session.user.id !== currentUser.id) {
                        if (categoriesCache.length === 0) { await getCategories(); } // Recharge si user change
                        handleUserConnected(session.user, false);
    ...
    function handleUserConnected(user, isInitialLoad) {
        ...
        // Cache catégories est géré par setupAuthListener et handleLogin

        if (!isInitialLoad && user.id !== previousUserId) {
            console.log("Nouvelle connexion détectée (utilisateur différent).");
            // Invalider le cache ici si ce n'est pas déjà fait par handleLogout
            if (categoriesCache.length > 0) invalidateCategoriesCache();
            getCategories(); // Recharge pour le nouvel utilisateur
    ...
    function handleUserDisconnected(isInitialLoad) {
        ...
        if (!isInitialLoad) {
            invalidateCategoriesCache(); // Vide le cache à la déconnexion
    ...
    function setActiveView(viewToShow, buttonToActivate){
        ...
        // Charger les données seulement si la vue est activée ET l'utilisateur est connecté (si nécessaire)
        if (viewToShow === searchView && chatHistory.length === 0) { displayWelcomeMessage(); }
        else if (viewToShow === inventoryView) { populateInventoryFilters(); displayInventory(); } // Inventaire visible par tous
    ...
    async function populateInventoryFilters() {
        ...
        try {
            const categories = await getCategories(); // Assure que le cache est chargé
    ...
    async function displayInventory(page = currentInventoryPage) {
        ...
        try {
            // Requête SANS jointure pour compatibilité Edge
            ...
            if (totalItems === 0) { if(inventoryNoResults) { inventoryNoResults.textContent = `Aucun composant trouvé${searchValue || categoryValue !== 'all' ? ' pour ces filtres' : ''}.`; inventoryNoResults.style.display = 'block'; } if(inventoryPageInfo) inventoryPageInfo.textContent = 'Page 0 / 0'; }
                ...
                // S'assurer que le cache est chargé (normalement déjà fait)
                if (categoriesCache.length === 0 && totalItems > 0) { // Vérifie si des items nécessitent le cache
                    await getCategories();
                }
                ...
                data.forEach(item => {
                    ...
                    // Recherche du nom de catégorie dans la map
                    const categoryName = categoryNameMap.get(item.category_id) || 'N/A';
                    row.insertCell().textContent = categoryName;
                    row.insertCell().textContent = item.drawer || '-'; row.insertCell().textContent = item.manufacturer || '-';
    ...
    async function displayLog(page = currentLogPage) {
        ...
        logTableBody.innerHTML = '<tr class="loading-row"><td colspan="6" style="text-align:center; color: var(--text-muted);"><i>Chargement historique...</i></td></tr>';
        ...
        try {
            ...
            else { if(logNoResults) logNoResults.style.display = 'none';
                data.forEach(entry => {
        ...
        } catch (err) { console.error("Erreur affichage historique:", err); logTableBody.innerHTML = `<tr><td colspan="6" class="error-message" style="color: var(--error-color); text-align: center;">Erreur chargement: ${err.message}</td></tr>`; if(logPageInfo) logPageInfo.textContent = 'Erreur'; }
    }
    ...
    // *** VERSION FINALE (sans jointure dans ce select pour compatibilité Edge) ***
    async function getStockInfoFromSupabase(ref) {
        if (!supabase || !ref) return null;
        const upperRef = ref.toUpperCase();
        console.log(`Supabase GET: Tentative récup pour ref: ${upperRef} (avec seuil, SANS jointure cat)`);
        try {
            const { data, error } = await supabase
                .from('inventory')
                .select('*, critical_threshold') // On garde cette version compatible
                .ilike('ref', upperRef)
                .single();
            ...
            if (error) {
                ...
                return null; // Gérer l'erreur en amont
            }
            console.log(`Supabase GET: Données reçues pour ${upperRef}:`, data);
            // data.categories sera undefined, il faut utiliser data.category_id
            return data;
        } catch (err) {
    ...
    // --- EXPORT MODIFIÉ POUR UTILISER LE CACHE CATEGORIES ---
    async function handleExportInventoryCSV() {
        if (!supabase) return;
        showSettingsFeedback('export', "Récup inventaire...", 'info');
        if(exportInventoryCsvButton) exportInventoryCsvButton.disabled = true;
        try {
            // Récupère SANS la jointure
            const { data, error } = await supabase.from('inventory').select('*, critical_threshold').order('ref', { ascending: true });
            if (error) throw new Error(`DB: ${error.message}`);
            if (!data || data.length === 0) { showSettingsFeedback('export', "Inventaire vide.", 'warning'); return; }

            // S'assurer que le cache catégories est chargé
            ...
            const csvData = data.map(item => ({
                ...
                drawer: item.drawer || '',
                category_name: categoryNameMap.get(item.category_id) || '', // Utilisation de la Map
            ...
            showSettingsFeedback('export', `Export CSV OK (${data.length} lignes).`, 'success');
        } catch (err) {
            console.error("Erreur export CSV:", err);
            showSettingsFeedback('export', `Erreur: ${err.message}`, 'error');
        } finally {
